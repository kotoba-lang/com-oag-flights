(ns oag_flights.main
  "Kotodama WASM entrypoint for the Oag_flights clean-room actor (L4) — Clojure port.
  ...
  facts (`oag_flights.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "oag_flights")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"oag_flights.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "oag_flights-compat" :tier tier :entities entities} 200])
...
(defn start! [] :oag_flights-compat/ready)
